...
/**
 * Explainable candidate ↔ job match score (Faza 8). Each factor contributes a
 * weighted number of points with a human-readable note, so recruiters see WHY
 * a candidate scored as they did — not just a black-box number. Total = 100.
 */
...
type CandidateLite = {
  ...
  city: string | null;
  expectedSalaryMax: unknown; // Prisma Decimal | null
...
export function computeJobMatch(candidate: CandidateLite, job: JobLite, relatedProfessionIds: Set<string>): MatchResult {
  ...
  {
    const max = 8;
    const needsField = contains(job.requirements, "teren") || contains(job.description ?? null, "teren");
