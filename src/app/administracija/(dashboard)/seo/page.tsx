...
export default async function AdminSeoPage() {
  ...
  for (const t of toolTrs) {
    check("TOOL", t.toolId, t.languageId, t.language.code, t.name, `/administracija/tools/${t.toolId}?lang=${t.language.code}`, {
      faqMissing: !Array.isArray(t.faqJson) || (t.faqJson as unknown[]).length === 0,
    });
  }
  for (const a of articleTrs) {
    check("ARTICLE", a.articleId, a.languageId, a.language.code, a.title, `/administracija/guides/${a.articleId}?lang=${a.language.code}`, {
      faqMissing: !Array.isArray(a.faqJson) || (a.faqJson as unknown[]).length === 0,
    });
  }
  for (const e of editorialTrs) {
    check("EDITORIAL", e.editorialPostId, e.languageId, e.language.code, e.title, `/administracija/editorial/${e.editorialPostId}?lang=${e.language.code}`);
  }
  for (const c of comparisonTrs) {
    check("COMPARISON", c.comparisonId, c.languageId, c.language.code, c.title, `/administracija/comparisons/${c.comparisonId}?lang=${c.language.code}`);
  }
  for (const n of newsTrs) {
    check("NEWS", n.newsItemId, n.languageId, n.language.code, n.title, `/administracija/news/${n.newsItemId}?lang=${n.language.code}`);
  }
  for (const p of promptTrs) {
    check("PROMPT", p.promptId, p.languageId, p.language.code, p.title, `/administracija/prompts/${p.promptId}?lang=${p.language.code}`);
  }
  for (const d of dealTrs) {
    check("DEAL", d.dealId, d.languageId, d.language.code, d.title, `/administracija/deals/${d.dealId}?lang=${d.language.code}`);
  }
  for (const p of pages) {
    check("PAGE", p.id, p.languageId, p.language.code, p.title, `/administracija/pages/${p.id}`);
  }
