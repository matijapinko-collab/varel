...
export default async function AdminPromptsPage() {
  ...
  return (
    <div>
      <PageHeader title="Prompts" action={{ href: "/administracija/prompts/new", label: "+ New prompt" }} />
      <AdminTable
          ...
          return (
            <tr key={p.id} className="hover:bg-background-secondary/50">
              <td className="px-4 py-3">
                <Link href={`/administracija/prompts/${p.id}`} className="font-medium hover:text-primary">
                  {main?.title ?? "(untitled)"}
                </Link>
              </td>
