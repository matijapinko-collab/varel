...
export default async function ToolOffersPage(props: PageProps<"/administracija/tools/[id]/offers">) {
  ...
  return (
    <div>
      <PageHeader title={`Offers: ${tool.name}`}>
        <Link href={`/administracija/tools/${tool.id}`} className="rounded-full border border-border bg-card px-4 py-2 text-sm font-medium hover:border-primary hover:text-primary">
          ← Back to tool
        </Link>
      </PageHeader>
      ...
      {partners.length === 0 && (
        <div className="mb-4 rounded-card border border-yellow-500/40 bg-yellow-500/10 px-4 py-3 text-sm">
          No affiliate partners yet. <Link href="/administracija/affiliate-partners/new" className="font-semibold text-primary">Create one first</Link>.
        </div>
      )}
