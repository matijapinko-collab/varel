    ...
    <div>
      ...
      <div className="mb-6 max-w-3xl rounded-card border border-border bg-card p-5 text-sm leading-relaxed">
        ...
        <p className="mt-3 text-xs text-muted">
          Slugs come from <Link href="/administracija/tools" className="text-primary hover:underline">Tools</Link> ({toolCount}) and{" "}
          <Link href="/administracija/affiliate-partners" className="text-primary hover:underline">Affiliate Partners</Link> ({partnerCount}).
          ...
          {recentOffers.map((o) => (
            <tr key={o.id}>
              <td className="px-4 py-2.5 text-sm font-medium">
                <Link href={`/administracija/tools/${o.toolId}/offers`} className="hover:text-primary">
                  {o.tool.name}
                </Link>
              </td>
