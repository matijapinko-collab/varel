...
import { requireElectroSuperadminForPasswordChange } from "@/lib/electro/auth/guard";
...
export default async function ElectroSaChangePasswordPage() {
  ...
  const forced = sa.mustChangePassword;

  return (
    <main className="mx-auto max-w-md px-6 py-8">
      {!forced && <Link href={ELECTRO_SUPERADMIN_BASE} className="text-sm text-muted hover:text-foreground">← Superadministracija</Link>}
      <div className={`mt-3 ${electroCardCls}`}>
        <h1 className="text-xl font-bold">Promjena lozinke</h1>
        <p className="mb-6 mt-1 text-sm text-muted">
          {forced
            ? "Prijavili ste se s privremenom lozinkom. Prije nastavka postavite novu lozinku."
            : "Preporučeno nakon prve prijave s privremenom lozinkom."}
        </p>
